<template>
    <div>
        <h1 class="text-center text-gray-800 pb-20 text-5xl font-extrabold">
            {{$t('Myprojects')}}
        </h1>
        <div class="flex flex-wrap bg-white rounded -mx-px overflow-hidden">

            <div class="w-full hover14 overflow-hidden sm:w-full md:w-1/2 lg:w-1/2 xl:w-1/2 border-b border-r">
                <div>
                    <a target="_blank" href="https://github.com/Houdaifi/Decathlon-voice">
                        <figure>
                            <img class="w-full" src="https://camo.githubusercontent.com/4c953b98717ca887a9a6ca90087dee4d409e6a22974551601731e54f88b66636/68747470733a2f2f692e6962622e636f2f4b3931523136372f30303030302d504f5254524149542d30303030302d425552535432303230303931313133313832353333312e6a7067" alt="Decathlon project">
                        </figure>
                    </a>
                </div>
            </div>

            <div class="w-full overflow-hidden sm:w-full md:w-1/2 lg:w-1/2 xl:w-1/2 border-b">
                <div class="flex flex-col justify-center items-center space-y-10 p-8 mt-8 ">
                    <div>
                        <h1 class="text-center text-fakhti font-bold text-3xl">Decathlon</h1>
                        <hr class="my-3">
                    </div>
                    <div>
                        <p style="max-width:500px" class="md:text-xl text-center text-gray-500 xl:leading-loose xl:tracking-tight">
                            Lorem ipsum dolor, sit amet consectetur adipisicing elit. Veritatis consequuntur aut alias reiciendis doloremque aperiam tempore vel sed voluptatem doloribus minus deserunt veniam et quibusdam expedita dolor, atque ex nam esse culpa cupiditate exercitationem. Natus, delectus.
                        </p>
                    </div>
                </div>
            </div>

            <div class="w-full overflow-hidden sm:w-full md:w-1/2 lg:w-1/2 xl:w-1/2 border-b border-r">
                <div class="flex flex-col space-y-10 justify-center items-center p-8 mt-8">
                    <div>
                        <h1 class="text-center text-fakhti font-bold text-3xl">Beko</h1>
                        <hr class="my-3">
                    </div>
                    <div>
                        <p style="max-width:500px" class="md:text-xl text-center text-gray-500 xl:leading-loose xl:tracking-tight">
                            Lorem ipsum dolor, sit amet consectetur adipisicing elit. Veritatis consequuntur aut alias reiciendis doloremque aperiam tempore vel sed voluptatem doloribus minus deserunt veniam et quibusdam expedita dolor, atque ex nam esse culpa cupiditate exercitationem. Natus, delectus.
                        </p>
                    </div>
                </div>
            </div>

            <div class="w-full hover14 overflow-hidden sm:w-full md:w-1/2 lg:w-1/2 xl:w-1/2 border-b">
                <div class="p-4">
                    <a target="_blank" href="https://houdaifi.github.io/Beko-website-clone/">
                        <figure>
                            <img class="w-full" src="../assets/beko.png" alt="Beko project">
                        </figure>
                    </a>
                </div>
            </div>

            <div class="w-full hover14 overflow-hidden cursor-pointer sm:w-full md:w-1/2 lg:w-1/2 xl:w-1/2 border-b border-r">
                <div>
                    <a target="_blank" href="https://play.google.com/store/apps/details?id=prinado.trivi">
                        <figure>
                            <img class="w-full " src="../assets/trivi.png" alt="Trivi project">
                        </figure>
                    </a>
                </div>
            </div>

            <div class="w-full overflow-hidden sm:w-1/2 md:w-1/2 lg:w-1/2 xl:w-1/2 border-b ">
                <div class="flex flex-col justify-center items-center space-y-10 p-8 mt-8">
                    <div>
                        <h1 class="text-center text-fakhti font-bold text-3xl">Trivi Templates</h1>
                        <hr class="my-3">
                    </div>
                    <div>
                        <p style="max-width:500px" class="md:text-xl text-center text-gray-500 xl:leading-loose xl:tracking-tight">
                            Lorem ipsum dolor, sit amet consectetur adipisicing elit. Veritatis consequuntur aut alias reiciendis doloremque aperiam tempore vel sed voluptatem doloribus minus deserunt veniam et quibusdam expedita dolor, atque ex nam esse culpa cupiditate exercitationem. Natus, delectus.
                        </p>
                    </div>
                </div>
            </div>

        </div>
        <div class="flex justify-center items-center pt-12">
            <button @click="goToProjects" class="bg-transparent hover:bg-turquoise font-semibold uppercase hover:text-white py-2 px-8 border border-turquoise hover:border-transparent rounded">
                {{$t('SeeMore')}}
            </button>
        </div>
      </div>
</template>


<script>
export default {
    name:'Projects',
    methods:{
        goToProjects(){this.$router.push({ name: 'Projects' })}
    }
}
</script>

<style scoped>
/* Shine */
.hover14 figure {
	position: relative;
}
.hover14 figure::before {
	position: absolute;
	top: 0;
	left: -75%;
	z-index: 2;
	display: block;
	content: '';
	width: 50%;
	height: 100%;
	background: -webkit-linear-gradient(left, rgba(255,255,255,0) 0%, rgba(255,255,255,.3) 100%);
	background: linear-gradient(to right, rgba(255,255,255,0) 0%, rgba(255,255,255,.3) 100%);
	-webkit-transform: skewX(-25deg);
	transform: skewX(-25deg);
}
.hover14 figure:hover::before {
	-webkit-animation: shine 1.5s;
	animation: shine 1.5s;
}
@-webkit-keyframes shine {
	100% {
		left: 125%;
	}
}
@keyframes shine {
	100% {
		left: 125%;
	}
}
</style>